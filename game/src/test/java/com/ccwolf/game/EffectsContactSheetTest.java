...
public class EffectsContactSheetTest {
    ...
    public void deathsUnfoldOverTime() throws IOException {
        ...
        sheet.frame.save("sheet-effects-deaths.png");
    }
    ...
    public void groundMarksAndTheEffectsWithoutARound() throws IOException {
        ...
        sheet.frame.save("sheet-effects-ground.png");
    }
    ...
    public void everyParticleKindAgainstGrass() throws IOException {
        ...
        sheet.frame.save("sheet-effects-particles.png");
    }
    ...
    private final class Sheet {
        ...
        Sheet(int columns, int rows, String[] headers) {
            ...
            frame = new Frame(GUTTER + columns * (CELL + PAD) + PAD,
                    LABEL_H + rows * (CELL + PAD) + PAD);
            canvas = frame.surface();
        ...
        void cell(int row, int col, Staging staging) {
            ...
            Frame cell = new Frame(CELL, CELL);
            Surface cellCanvas = cell.surface();
            ...
            canvas.drawImage(cell.image(), x, y, x + CELL, y + CELL, blit);
        ...
        private boolean differsFromGrass(Frame cell) {
            int off = 0;
            for (int y = 0; y < CELL; y++) {
                for (int x = 0; x < CELL; x++) {
                    int c = cell.pixel(x, y);
